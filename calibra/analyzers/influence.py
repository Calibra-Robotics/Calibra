...
class InfluenceAnalyzer(Analyzer):
    ...
    def analyze(
        ...
        # 2. Entropy scores
        entropy_scores = [compute_trajectory_entropy(ep.actions) for ep in episodes]
        ...
        per_ep_dict = {
            episodes[i].metadata.episode_id: float(influence_scores[i]) for i in range(n)
        }
        ...
        raw = {
            ...
            "per_episode_novelty": {
                episodes[i].metadata.episode_id: float(norm_novelty[i]) for i in range(n)
            },
            "per_episode_entropy": {
                episodes[i].metadata.episode_id: float(norm_entropy[i]) for i in range(n)
            },
            "per_episode_contact": {
                episodes[i].metadata.episode_id: float(norm_contact[i]) for i in range(n)
            },
        }
